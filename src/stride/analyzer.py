...
def infer_severity(component: str, stride_category: str) -> str:
    high_risk_components = {"database", "api", "gateway", "identity_provider"}
    high_risk_categories = {"Information Disclosure", "Elevation of Privilege", "Spoofing"}
...
def analyze_stride(detections: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    components = sorted(set(item["component"] for item in detections))
    report = []
